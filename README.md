...
Core implementation is in place: host-native project config, automatic LLM task decomposition with single-task fallback, async expert dispatch, semantic synthesis, an awaited HOTL decision gate in the background job (approve, reject, or edit-and-resynthesize), durable session-backed audit persistence, a host model-registry → Mnemopi → Ollama embedding chain, bundled per-role agent personas (`legion-coder`/`legion-reviewer`/`legion-tester`/`legion-generalist`/`legion-scout`, plus an internal `legion-decomposer` planning-only persona that is never itself dispatchable) with a native-`task`-tool guard, an `irc` guard, and a `git commit` guard so they're only reachable through governed dispatch, can't coordinate with sibling attempts, and can't land unreviewed commits on their own, an auto-discovered usage rule (`rules/legion-dispatch.md`) teaching the primary agent when to reach for it, a `/skill:centurion` ensemble-driven clarifying-question loop for high-stakes design decisions, and a tree-style TUI card showing each task's own attempts/models nested under it. Every attempt now runs isolated (copy-on-write, host-native) with winner-only merge-back onto the real repo, and a concurrency cap bounds total in-flight attempts per dispatch — see `docs/plan/algorithm-audit-and-hardening-v2.md` for the full audit and hardening roadmap this is being built against. The live benchmark harness is `scripts/benchmark.ts`; run it against your configured models for manual ensemble versus single-model comparison.
