...
**Status:** core dispatch, automatic decomposition, synthesis, HOTL governance with an awaited background decision gate, host-native config, extension entrypoint, durable Legion audit persistence, explicit registry-level embeddings, bundled per-role agent personas (`legion-coder`/`legion-reviewer`/`legion-tester`/`legion-generalist`/`legion-scout`, plus an internal `legion-decomposer` planning-only persona never itself dispatchable, with a native-`task` guard, an `irc` guard, and a `git commit` guard preventing them from being reached or coordinating outside governed dispatch), an auto-discovered usage rule (`rules/legion-dispatch.md`), a `/skill:centurion` ensemble-driven clarifying-question loop for high-stakes design decisions, and a framed-block dispatch result card (per-task attempt/model nesting, live progress widget) are implemented. A live real-model comparison harness is available at `scripts/benchmark.ts`; benchmark results remain user-run and are intentionally not claimed here. Every section below states what v1 does and, where relevant, explicitly what it does not — the project this replaced accumulated four rounds of planning docs that drifted from its actual implementation, and the discipline of this spec is to never repeat that.

**For a detailed, file-by-file account of how the code implements every decision below, see [`docs/ARCHITECTURE.md`](../ARCHITECTURE.md).**
