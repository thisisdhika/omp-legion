...
2. **Dispatch.** Call `legion_dispatch` with an explicit task, skipping auto-decomposition since the role is already known:
   ```
   tasks: [{ id: "scout-<round>", role: "scout", description: "Choose the sharpest unresolved question.", assignment: "<the assignment from step 1>" }]
   ```
3. **Wait for the blocking result.** `legion_dispatch` blocks until decomposition, expert execution, synthesis, and any human governance resolve, then returns the synthesized result. Do not draft a question, options, or recommendation before that result is returned; there is no interim job to poll.
4. **When the call returns, present exactly that result.** The question, options, and recommendation you show the human must be the ensemble's own synthesized text — quote or closely paraphrase it, don't reconstruct it from memory of what you expected it to say. Attribute it clearly as the ensemble's output. Wait for the human's answer before continuing. Asking multiple questions at once, or moving on before they've answered, is exactly the failure mode grilling exists to avoid.

   **A `Legion escalation for <job>/<task>: <reasons>` notice (e.g. `confidence, disagreement, cost`) is not a failure — do not treat it as one.** It means the ensemble ran, produced a real synthesized answer, and HOTL flagged it for human visibility because confidence was low, the experts disagreed, or cost ran high. The delivered result still carries a genuine synthesized question/options/recommendation — present it exactly as step 4 says, and add one line naming which governance reason(s) triggered the flag so the human can weigh the recommendation accordingly (e.g. "flagged for low confidence — the ensemble disagreed on this one"). **Never invent an unrelated explanation for why the round "failed"** (e.g. a claimed model-unavailability) when the actual delivered signal was a governance escalation, and never fall back to a self-generated question instead of the delivered one — that violates the entire premise of this skill.

   If a round's dispatch truly *did* fail outright (no synthesis at all — a genuine tool/model error, not an escalation), say exactly that, quote the real error text you were given, and ask the human whether to retry or fall back to the regular grilling skill. Don't guess at a cause you weren't actually told.
